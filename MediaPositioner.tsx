...
export const MediaPositioner: React.FC<MediaPositionerProps> = ({ 
  className = '', 
  onPositionSelected,
  mediaType = 'video'
}) => {
  ...
  const drawMap = () => {
    ...
    if (selectedPosition) {
      ...
      ctx.fillStyle = mediaType === 'video' ? '#FF5722' : '#9C27B0';
      ctx.beginPath();
      ...
      // Dessiner une icône au centre
      ctx.fillStyle = '#ffffff';
      ctx.font = '10px Arial';
      ctx.textAlign = 'center';
      ctx.textBaseline = 'middle';
      ctx.fillText(mediaType === 'video' ? '▶' : '♪', pos.x, pos.y);
    }
  };
  ...
  return (
    <div className={`p-4 ${className}`}>
      <h2 className="text-lg font-medium mb-4">
        Positionner le {mediaType === 'video' ? 'média vidéo' : 'média audio'}
      </h2>
      
      <div className="bg-white dark:bg-gray-800 p-3 rounded-lg shadow-md">
        <p className="text-sm text-gray-600 dark:text-gray-400 mb-2">
          Cliquez sur la carte pour choisir la position du {mediaType === 'video' ? 'média vidéo' : 'média audio'}.
        </p>
